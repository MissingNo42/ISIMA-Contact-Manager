	...
	internal class Program
	{
		...
		private bool Remove(string[] args)
		{
			if (args.Length == 3)
			{
				...
				{
					case "folder":
						if (args[2] == ".") manager.RemoveCurrent();
						else if (!manager.RemoveFolder(args[2])) Console.WriteLine($"Remove: folder \"{args[2]}\" does not exist");
						break;
				...
				switch (args[1])
				{
					...
					case "contact":
						if (args.Length >= 4)
						{
							...
							if (contact == null) Console.WriteLine($"Update: contact \"{args[2]}\" does not exist");
								...
								for (int i = 3; i < args.Length; i++)
								{
									if (!args[i].Contains('='))
									{
										Console.WriteLine($"Update: invalid argument, requested key=value argument, got {args[i]}");
										return false;
									}
									...
									switch (arg[0])
									{
										case "name":
											...
											{
												contact.Name = arg[1];
												Console.WriteLine($"Update: contact name updated to {arg[1]}");
											}
										...
										case "firstname":
											try
											{
												...
												Console.WriteLine($"Update: contact first updated to {arg[1]}");
											}
											catch (ArgumentException e)
											{
												Console.WriteLine($"Update: {e.Message}");
										...
										case "company":
											try
											{
												...
												Console.WriteLine($"Update: contact company updated to {arg[1]}");
											}
											catch (ArgumentException e)
											{
												Console.WriteLine($"Update: {e.Message}");
										...
										case "relation":
											if (Enum.TryParse(CultureInfo.CurrentCulture.TextInfo.ToTitleCase(arg[1]), out Relation relation))
											{
												contact.Relation = relation;
												Console.WriteLine($"Update: contact relation updated to {relation}");
											}
										...
										case "email":
											try
											{
												...
												Console.WriteLine($"Update: contact email updated to {arg[1]}");
											}
											catch (FormatException)
											{
										...
										default:
											Console.WriteLine($"Update: invalid argument, requested a valid key for key=value argument, got {arg[0]}");
											return true;
									}
								}
						}
		...
		private bool Load(string[] args)
		{
			...
			try
			{
				if (manager.Load(key)) Console.WriteLine("Load: loading successful");
				else if (manager.TryCount <= 0) Console.WriteLine("Load: too many invalid password: deleting the database...");
				else Console.WriteLine($"Load: loading failed: Invalid password, {manager.TryCount}/3 remaining tries");
		...
		{
			return args.Length > 1 ? true : throw new Exception("Exit");
		}
	}
}
